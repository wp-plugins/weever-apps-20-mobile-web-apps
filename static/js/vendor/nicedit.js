...
var nicEditorConfig = bkClass.extend({
	...
	iconsPath : '../wp-content/plugins/weever-server/admin/assets/static/js/vendor/nicEditorIcons.gif',
...
var nicLinkButton = nicEditorAdvancedButton.extend({
	addPane : function() {
		...
		this.addForm({
			...
			'target' : {type : 'select', txt : 'Open In', options : {'' : 'Current Window', '_blank' : 'New Window'},style : {width : '100%'}}
		},this.ln);
	},
...
var nicImageButton = nicEditorAdvancedButton.extend({
	addPane : function() {
		...
		this.addForm({
			...
			'alt' : {type : 'text', txt : 'Alt Text', style : {width: '100%'}},
...
var nicUploadButton = nicEditorAdvancedButton.extend({
	...
	addForm : function() {
		...
		myDoc.write('<form method="post" action="'+url+'" enctype="multipart/form-data">');
	...
	startUpload : function( e ) {
		...
		var url = this.uri;
		url += ( url.indexOf( '?' ) >= 0 ) ? '&' : '?';
		url += 'rand=' + Math.round(Math.random()*Math.pow(10,15)) ;

		jQuery.ajax( url, {
			method: 'POST',
			iframe: true,
			files : jQuery( e.currentTarget ),
			success: function( data ) {
				console.log( data );

				// The stupid data comes in with HTML for some reason
				...
				if ( data.error_code ) {
					// Error!
					alert( 'Error Code ' + data.error_code + ': ' + data.error_message );
				}
				else {
					// Success! Create an image, and add it to the editor.
					me.ne.selectedInstance.restoreRng();
					var tmp = 'javascript:nicImTemp();';
					me.ne.nicCommand("insertImage", tmp);
					me.im = me.findElm('IMG', 'src', tmp);

					if(me.im) {
						me.im.setAttributes({ src : data.file_name });
					}
				}
