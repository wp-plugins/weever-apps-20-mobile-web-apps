...
// Brian Hogg - Class renamed to try and avoid conflict with other Wordpress plugins
class WeeverMdetect
{
   var $useragent = "";
   var $httpaccept = "";
   ...
   var $deviceBBBoldTouch97 = 'blackberry 979'; //### Bold Touch
   ...
   {
	  // ### UGLY HACK, NEEDS BETTER FIX! 
	  // Remove the webkit check in appbuilder!
	  // Reason why we did this fix this way, is because the plugin itself sets ignore_mobile to 1. 
	  // But we need to start redirecting windows phone 8. So this one variable in session was making 
	  // our plugin not continue with checking the phone's capabilities.
	  // So, in sum, setting this to 0, WP8 is no longer ignored, and goes through MobileESP for detection.
	  if (stripos($this->useragent, $this->deviceWinPhone8) > -1)
		 $_SESSION['ignore_mobile'] = 0;
      if (stripos($this->useragent, $this->deviceWinPhone8) > -1)
         return $this->true; 
      else
         return $this->false; 
   ...
   function DetectWindowsIE11()
   {
      if (stripos($this->useragent, $this->deviceWinIE11) > -1)
         return $this->true;
      else
         return $this->false;
   }
   ...
   {
      if (stripos($this->useragent, $this->deviceWinIE10) > -1)
         return $this->true;
      else
         return $this->false;
   }
   ...
   // Focuses on Windows Mobile 6.xx and earltr.
  ...
  //*****************************
   ...
   // ### Edited to follow Weever's ideals
